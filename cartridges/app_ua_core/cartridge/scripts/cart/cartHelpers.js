...
function bopisLineItemInventory(currentBasket, changeAllItemsStore, cartProdPid, cartProdUUID) {
    ...
    try {
        ...
        if (isMAOEnabled && realTimeInventoryCallEnabled && isBopisCheckPointEnabled && storeObj.ID) {
            collections.forEach(inStoreShimpent.productLineItems, function (PLI) {
                if (PLI.product.custom.availableForInStorePickup !== false && PLI.custom.fromStoreId) {
        ...
        collections.forEach(inStoreShimpent.productLineItems, function (PLI) {
            ...
            Transaction.wrap(function () {
                if (instoreInventory > 0 && PLI.custom.fromStoreId) {
